...
(defn on-add [props new-tag]
  (let [{:keys [panel-opened transform-tag-fn tags input-value on-change]} props
        ...
        new-tags (into [] (concat (h/ref-or-val tags) [item]))]

    (when (satisfies? IDeref tags)
      (reset! tags new-tags))
    (reset! input-value "")
...
(defn matching-items [props items]
  (let [{:keys [tags input-value]} props]
    (->>
      ...
      (filterv (fn [i] (not-any? #(= (cue/lower (category-item-id %)) (cue/lower (category-item-id i))) (h/ref-or-val tags)))  ))))
...
(defn add-selected-tag [props]
  (let [{:keys [add-new tags input-value panel-opened categories selection]} props
        full-match (full-match-in-items input-value (h/ref-or-val tags))]
    (when-not (or
                (and
                  (not @panel-opened)
                  (> (count @input-value) 0))
...
(defn category [{:keys [hide-single-category categories tags items title selected selected-item input-value] :as props}]
  ...
  (let [matching-items (matching-items props items)
        full-match     (boolean (full-match-in-items input-value items (h/ref-or-val tags)))
...
(defn handle-backspace [props e]
  (let [{:keys [input-value tags on-tag-deleted]} props]
    (when (= (count (cue/trim @input-value)) 0)
      (.preventDefault e)
      (on-tag-deleted (- (count (h/ref-or-val tags)) 1)))))
...
(defn tag-input-area [props]
  ...
  (fn [{:keys [input-value min-chars categories tags on-tag-deleted panel-opened tag-style-fn placeholder] :as props }]
  ...
  (let [panel-opened (r/atom false)
    ...
    (fn [& {:keys [add-new
            ...
            :or {add-new true
                 ...
                 on-change (fn [tags]
                             (log/info "got these tags " (pr-str tags)))
...
(defn tag-list [& {:keys [tags tag-icon-class]
                   :or {tag-icon-class "fa fa-tags"}}]
  [:p.form-control-static
   [:i {:class tag-icon-class}] " "
   (str/join ", " tags)
